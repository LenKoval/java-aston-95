...
public class WildcardPECS {
    ...
    private static void printProducer(List<? extends Cat> catList) {
        // catList.add(new Object()); //Ошибка логично
        // catList.add(new Animal()); //Ошибка логично
        // catList.add(new Cat()) //Ошибка а почему??
        // catList.add(new HomeCat("f")); //Ошибка компилятор не знает какие объекты придут
    ...
    private static void printConsumer(List<? super Cat> catList) {
        //catList.add(new Object());// Ошибка а почему???
        //catList.add(new Animal());// Ошибка
        // super означает что в нашей коллекции гарантированно будут лежать объекты Cat и выше по иерархии
        // но не запрещает нам добавлять в коллекцию наследников Cat
        catList.add(new Cat());
